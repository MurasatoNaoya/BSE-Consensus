...
class Trader_PRZI(Trader):
    ...
    def __init__(self, ttype, tid, balance, params, time):
        ...
        if type(params) is dict:
            ...
            f = None
            t = 7200 # Default value.

            if 'f' in params: # Check if 'f' is present in the dictionary
                f = params['f']

            if 't' in params: # Check if 't' is present in the dictionary
                t = params['t']
        ...
        self.strat_wait_time = t     # how many secs do we give any one strat before switching?
        ...
        if ttype == 'PRSH' or ttype == 'PRDE' or ttype == 'PRZI':
            ...
            if mapping:
                ...
                if ttype == 'PRSH':
                    ...
                    parameters = {'optimizer': 'PRDE','f': trader_params['f'], 'k': trader_params['k'], 't':trader_params['t'],
                                  'strat_min': trader_params['s_min'], 'strat_max': trader_params['s_max']}
                else: # ttype=PRZI
...
def market_session(sess_id, starttime, endtime, trader_spec, order_schedule, avg_bals, dump_all, verbose, session_seed):
      
    global seed
    seed = session_seed
    print(f'The main seed used for this market session is {seed}')

    random.seed(seed)

    global master_random
