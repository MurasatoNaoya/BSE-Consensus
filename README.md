...
Single node; no P2P, no multi-block chain, no difficulty retargeting. A run is fully determined by its seed, so seeds should be bound to external randomness (e.g. a prior block hash) to prevent offline grinding. The market model is intentionally minimal: just enough that fitness varies with strategy.
